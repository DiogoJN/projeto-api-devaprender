...
def token_obrigatorio(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not token:
            return jsonify({'mensagem':'Token não foi incuído!'}, 401)
...
def login():
    ...
    if not auth or not auth.username or not auth.password:
        return make_response('Login inválido',401,{'WWW-Athenticate':'Basic realm="Login obrigatório"'})
...
def obter_postagens(autor):
    ...
    return jsonify({'mensagem':lista_postagens})

# obter postagem com id  http://localhost:5000/postagem/1
@app.route('/postagem/<int:id_postagem>', methods=['GET'])
@token_obrigatorio
def obter_postagem_por_indice(autor, id_postagem):
    postagem = Postagem.query.filter_by(id_postagem=id).first()
    postagem_atual = {}
    try:
        postagem_atual['titulo'] = postagem.titulo
    except:
...
# Excluir um recurso - DELETE http://localhost:5000/postagem/0
@app.route('/postagem/<int:id_postagem>', methods=['DELETE'])
@token_obrigatorio
def excluir_postagem(autor, id_postagem):
    postagem_a_ser_exlcuida = Postagem.query.filter_by(id_postagem=id_postagem).filter_by()
    if not postagem_a_ser_exlcuida:
        return jsonify({'mensagem': 'Não foi encontrado uma mensagem com este id'})
    db.session.delete(postagem_a_ser_exlcuida)
    db.session.commit()
...
def obter_autor_por_id(autor, id_autor):
    ...
    if not autor:
        jsonify(f'Autor não encontrado!')
    autor_atual = {}
    autor_atual['id_autor'] = autor.id_autor
    autor_atual['nome'] = autor.nome
    autor_atual['email'] = autor.email
...
def excluir_autor(autor, id_autor):
    ...
    return jsonify({'mensagem':'Autor exluído com sucesso'}, 200)
